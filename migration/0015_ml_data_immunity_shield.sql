-- Migration 0015: ML Training Data Immunity & Decoupling Shield (Phase 10.2)
--
...
--
-- Actions:
-- 1. Alter ai_training_conversations:
--    - Drop existing ON DELETE CASCADE foreign key constraints.
--    - Make merchant_id and conversation_id NULLABLE.
--    - Re-add foreign keys with ON DELETE SET NULL.
--    - Add immutable merchant_cohort_hash (SHA-256 fingerprint of merchant/category).
--    - Add anonymized_actor_token (synthetic session token, PII-free).
-- 2. Add indexes for cohort-based dataset export.
