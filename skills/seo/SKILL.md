...
3. If Google API credentials detected (`"$HOME/.claude/skills/seo/scripts/claude-seo" run google_auth.py --check`), also spawn seo-google agent
...
6. If backlink APIs detected (`"$HOME/.claude/skills/seo/scripts/claude-seo" run backlinks_auth.py --check`), also spawn seo-backlinks agent
...
10. If drift baseline exists for this URL (`"$HOME/.claude/skills/seo/scripts/claude-seo" run drift_history.py <url>`), also spawn seo-drift agent
